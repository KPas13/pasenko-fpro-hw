...
let checkoutBox = document.querySelector('#order-info');
let valueProductSpan = document.querySelector('#value-product');
...
function createBuyButton(productArray, productDiv, infoDiv, categoriesDiv) {
    ...
    buyButton.addEventListener('click', () => {
        ...
        let confirmButton = document.querySelector('#order-confirm-btn');

        confirmButton.removeEventListener('click', checkout);
        function checkout() {
            ...
            let userPoshta = document.querySelector('#user-poshta').value;
            let userPayment = document.getElementsByName('payment');
            let userValue = document.querySelector('#user-value').value;
            let userComment = document.querySelector('#comment-text').value;

            function isPaymentSelected() {
                for (let i = 0; i < userPayment.length; i++) {
                    if (userPayment[i].checked) {
                        return true;
                    }
                }
                return false;
            ...
            if (!userName || !userSurname || !userPatronymic || !userCity || !userPoshta || !isPaymentSelected() || !userValue) {
                alert('Fill in all fields with *');
            } else {
                Customer.name = userName;
                Customer.surname = userSurname;
                Customer.patronymic = userPatronymic;
                Customer.city = userCity;
                Customer.postOffice = userPoshta;
                for (let i = 0; i < userPayment.length; i++) {
                    if (userPayment[i].checked) {
                        Customer.payment = userPayment[i].value;
                    }
                }
                Customer.value = userValue;
                Customer.comments = userComment;

                if (localStorage.getItem(`${productArray.productName}`) !== null) {
                    let storedProductData = JSON.parse(localStorage.getItem(`${productArray.productName}`));
                    storedProductData.productAmount = parseInt(Customer.value);
                    localStorage.setItem(`${productArray.productName}`, JSON.stringify(storedProductData));
                } else {
                    productArray.productAmount = parseInt(Customer.value);
                    localStorage.setItem(`${productArray.productName}`, JSON.stringify(productArray));
                }

                alert('You successfully buy');

                alert(`Order info:\n Name: ${Customer.name}\n Surname: ${Customer.surname}\n Patronymic: ${Customer.patronymic}\n City: ${Customer.city}\n Post Office: ${Customer.postOffice}\n Payment: ${Customer.payment}\n Value of product: ${Customer.value}\n Comment to order: ${Customer.comments}\n Product Name: ${productArray.productName}`);
...
function orderShow(categoriesDiv, orderBtn, infoDiv, productDiv) {
